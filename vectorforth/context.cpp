...
context create_context(uint64_t stack_size_in_bytes, uint64_t data_space_size_in_bytes)
  {
  const uint64_t alignment = 256; // must be a power of two
  context c;
  c.memory_allocated = (char*)malloc(stack_size_in_bytes + alignment - 1 + data_space_size_in_bytes);
  ...
  c.here_pointer = c.aligned_stack_top;
  c.data_space_pointer = c.here_pointer+32;

  *((uint64_t*)c.here_pointer) = (uint64_t)((void*)c.data_space_pointer);
